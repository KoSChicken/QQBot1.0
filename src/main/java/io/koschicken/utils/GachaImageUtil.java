...
public class GachaImageUtil {
    ...
    private GachaImageUtil() {
    }

    //制作抽卡出货的图
    public static String composeImg(List<String> characters) throws IOException {
        if (!characters.isEmpty()) {
            ...
            for (int i = 0; i < characters.size(); i++) {
                ...
                if (file.exists()) { //如果外部图片包 有资源则使用外部资源
                    ...
                    InputStream inputStream = GachaImageUtil.class.getResourceAsStream("/image/" + characters.get(i) + ".png");
                    if (inputStream == null) { //内部资源也没有就显示问号占位图片
                        inputStream = GachaImageUtil.class.getResourceAsStream("/image/no.png");
                    }
